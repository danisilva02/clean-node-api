...
import { MissingParamName, ServerError, InvalidParamError } from '../../erros'
...
describe('SignUp Controller', () => {
  ...
  test('Should return 400 if password is diferent the confirmation', async () => {
    const { sut } = makeSut()
    const httpRequest = {
      body: {
        name: 'any_name',
  ...
  test('Should return 400 if an invalid email is provided', async () => {
    ...
    expect(isValidSpy).toHaveBeenCalledWith('any_email_invalid')
  })
  ...
  test('Should call EmailValidator with correct email', async () => {
    ...
    const httpRequest = {
      body: {
        name: 'any_name',
        email: 'any_email_valid',
        password: 'any_password',
        passwordConfirmation: 'any_password'
      }
    }
    await sut.handle(httpRequest)
    expect(isValidSpy).toHaveBeenCalledWith('any_email_valid')
  })
